...
1. **Frontend (`talentflow-web`):**
   - File: [`src/components/PDFViewer.tsx`](file:///Users/laurielmesquita/Space%20Square/02-Customers/TalentFlow/05-Projetos/talentflow-web/src/components/PDFViewer.tsx)
   - Monta a URL: `${API_URL}/api/candidates/${candidateId}/pdf?token=${authToken}`
   - Lê o token via `getCookie("token")`.
2. **Backend (`talentflow-api`):**
   - File: [`app/api/candidates.py`](file:///Users/laurielmesquita/Space%20Square/02-Customers/TalentFlow/05-Projetos/talentflow-api/app/api/candidates.py) (endpoint `GET /api/candidates/{candidate_id}/pdf`)
   - File: [`app/api/deps.py`](file:///Users/laurielmesquita/Space%20Square/02-Customers/TalentFlow/05-Projetos/talentflow-api/app/api/deps.py) (`get_current_user` aceita `?token=`)
