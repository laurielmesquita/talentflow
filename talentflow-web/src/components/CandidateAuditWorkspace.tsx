...
import { 
  ...
  AlertTriangle, Briefcase, Award, FileCheck2, Trash2, UserCheck 
} from "lucide-react";
...
export default function CandidateAuditWorkspace({ candidateId }: CandidateAuditWorkspaceProps) {
  ...
  const tierConfigs = {
    high:   { Icon: ShieldCheck, color: "text-emerald-400", bar: "bg-emerald-500", label: "Alta Compatibilidade & Organização", border: "border-emerald-500/30", bg: "bg-emerald-500/10" },
    medium: { Icon: ShieldAlert, color: "text-amber-400",   bar: "bg-amber-500",   label: "Atenção Moderada", border: "border-amber-500/30",   bg: "bg-amber-500/10" },
    low:    { Icon: ShieldX,     color: "text-rose-400",    bar: "bg-rose-500",    label: "Baixa Organização / Informações Ausentes", border: "border-rose-500/30",    bg: "bg-rose-500/10" },
  };
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-slate-950 text-slate-100 overflow-hidden font-sans">
      ...
      <AnimatePresence>
        {actionMessage && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="fixed top-3 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 px-4 py-2 rounded-full border shadow-2xl bg-slate-900/95 backdrop-blur border-slate-700 text-xs font-semibold tracking-wide"
          >
            ...
            <span className="text-slate-200">{actionMessage.text}</span>
          </motion.div>
        )}
      </AnimatePresence>

      {/* ── CABEÇALHO DO WORKSPACE (HEADER) ────────────────────────────────── */}
      <header className="flex items-center justify-between px-6 py-3 bg-slate-900/90 border-b border-slate-800/80 backdrop-blur-md z-20 shadow-sm flex-shrink-0">
        <div className="flex items-center space-x-4">
          <button
            onClick={() => router.push("/dashboard")}
            className="flex items-center space-x-2 px-3 py-1.5 rounded-lg bg-slate-800/80 hover:bg-slate-800 text-slate-300 hover:text-white transition-all text-xs font-medium border border-slate-700/60"
          >
        ...
        <div className="flex items-center space-x-2 bg-slate-950/80 p-1 rounded-xl border border-slate-800/80 shadow-inner">
          <button
            onClick={() => prevCandidate && navigateTo(String(prevCandidate.id))}
            disabled={!prevCandidate || loading}
            className="flex items-center space-x-1 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all hover:bg-slate-800 disabled:opacity-40 disabled:hover:bg-transparent text-slate-300 disabled:cursor-not-allowed"
          >
            <ChevronLeft className="w-4 h-4 text-indigo-400" />
            <span>Anterior</span>
          </button>
          <span className="text-xs text-slate-500 font-mono font-medium px-2 border-x border-slate-800">
        ...
        <div className="flex items-center space-x-2">
          <button
            ...
            className="flex items-center space-x-1.5 px-3.5 py-1.5 rounded-lg bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/30 transition-all text-xs font-semibold shadow-sm"
          >
          ...
          <button
            ...
            disabled={loading || !candidate}
            className="flex items-center space-x-1.5 px-4 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white transition-all text-xs font-semibold shadow-md hover:shadow-indigo-500/20"
          >
      ...
      <main className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-4 p-4 overflow-hidden bg-slate-950/50">
        {loading ? (
          <div className="col-span-1 lg:col-span-2 flex flex-col items-center justify-center h-full text-indigo-400">
            <div className="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-indigo-400 mb-4" />
            <span className="text-sm font-semibold tracking-wide animate-pulse text-slate-300">
          ...
          <div className="col-span-1 lg:col-span-2 flex flex-col items-center justify-center h-full text-center p-8">
            ...
            <h3 className="text-lg font-bold text-slate-200">Erro na Auditoria</h3>
            <p className="text-sm text-slate-400 mt-1 max-w-md">{error || "Candidato não encontrado no sistema."}</p>
            <button
              onClick={() => router.push("/dashboard")}
              className="mt-6 px-5 py-2 rounded-xl bg-slate-800 text-slate-300 hover:bg-slate-700 text-sm font-medium transition-all"
            >
              Voltar ao Painel
            </button>
          </div>
          ...
          <>
            ...
            <div className="flex flex-col h-full w-full overflow-y-auto pr-1 rounded-xl border border-slate-800/80 bg-slate-900/90 shadow-xl p-6 custom-scrollbar">
              ...
              {candidate.is_flagged && (
                <div className="mb-6 p-4 rounded-xl border border-rose-500/30 bg-rose-500/10 flex items-start gap-3 shadow-inner">
                  <ShieldAlert className="w-5 h-5 mt-0.5 shrink-0 text-rose-400" />
                  <div>
                    <h4 className="text-sm font-bold text-rose-200">Perfil Sinalizado (Blacklist)</h4>
                    <p className="text-xs text-rose-300/80 mt-1 leading-relaxed">{candidate.flagged_reason}</p>
                    {candidate.flagged_at && (
                      <p className="text-[10px] text-rose-400/70 mt-2 font-mono uppercase">
              ...
              {flagging && !candidate.is_flagged && (
                <motion.div 
                  ...
                  className="mb-6 p-4 rounded-xl border border-amber-500/30 bg-amber-500/10 text-slate-200 space-y-3"
                >
                  <div className="flex items-center gap-2 text-amber-300 font-semibold text-xs uppercase tracking-wide">
                  ...
                  <textarea
                    ...
                    className="w-full h-24 p-3 rounded-lg border border-slate-800 bg-slate-950 text-xs text-slate-200 placeholder-slate-600 focus:outline-none focus:border-rose-500/50 resize-none"
                  />
                  <div className="flex justify-end gap-2">
                    <button
                      onClick={() => { setFlagging(false); setFlagReason(""); }}
                      disabled={submittingFlag}
                      className="px-3 py-1.5 rounded-lg text-xs font-medium text-slate-400 bg-slate-800 hover:bg-slate-700 transition-all"
                    >
                      Cancelar
                    </button>
                    <button
              ...
              <div className="flex items-start gap-4 pb-6 border-b border-slate-800/80">
                {candidate.photo_url ? (
                  <img
                    ...
                    className="w-16 h-16 rounded-2xl object-cover border-2 border-indigo-500/40 shrink-0 shadow-md"
                  />
                ) : (
                  <div className="w-16 h-16 rounded-2xl bg-indigo-600/20 border-2 border-indigo-500/40 flex items-center justify-center text-xl font-bold text-indigo-400 uppercase shrink-0 shadow-md">
                    {candidate.full_name.substring(0, 2)}
                  </div>
                )}
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between">
                    <h1 className="text-xl font-bold text-white truncate">{candidate.full_name}</h1>
                    {candidate.added_at && (
                      <span className="text-[10px] text-slate-400 font-mono bg-slate-800/60 px-2 py-1 rounded-md border border-slate-700/40">
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-3">
                    {candidate.categories?.map((c) => (
                      <span key={c} className="text-[11px] font-semibold text-slate-300 bg-slate-800/90 px-2.5 py-0.5 rounded-md border border-slate-700/50">
                        {c}
                      </span>
                    ))}
                  </div>
              ...
              {/* CONTATOS EXTRAÍDOS */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 py-4 border-b border-slate-800/80 text-xs">
                {candidate.email && (
                  <div className="flex items-center gap-2.5 text-slate-300 bg-slate-950/60 p-2.5 rounded-xl border border-slate-800/50 truncate">
                ...
                {candidate.address && (
                  <div className="col-span-1 md:col-span-2 flex items-center gap-2.5 text-slate-300 bg-slate-950/60 p-2.5 rounded-xl border border-slate-800/50 truncate">
              ...
              <div className={`mt-6 p-5 rounded-2xl border ${currentTier.border} ${currentTier.bg} backdrop-blur shadow-sm`}>
                ...
                <div className="flex items-center justify-between mb-2">
                  <div className={`flex items-baseline gap-1 font-extrabold text-2xl ${currentTier.color} font-mono`}>
                    ...
                    <span className="text-xs font-medium text-slate-400">/ 100 PTS</span>
                  </div>
                ...
                <div className="w-full bg-slate-950 rounded-full h-2 mb-4 border border-slate-800/60 overflow-hidden">
                  <div
                ...
                {candidate.quality_alerts && candidate.quality_alerts.length > 0 && (
                  <div className="mt-4 pt-3 border-t border-slate-800/60 space-y-2">
                    <p className="text-[11px] text-amber-400/90 font-semibold uppercase tracking-wider flex items-center gap-1.5">
                    ...
                    {candidate.quality_alerts.map((alert, i) => {
                      ...
                      return (
                        <div key={i} className="flex items-start gap-2 text-xs text-slate-300 bg-slate-950/40 p-2 rounded-lg border border-amber-500/10">
                ...
                <div className="flex flex-wrap gap-2">
                  {candidate.skills && candidate.skills.length > 0 ? (
                    candidate.skills.map((skill) => (
                      <span
                        key={skill}
                        className="px-3 py-1 rounded-xl text-xs font-semibold bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 shadow-sm hover:bg-indigo-500/20 transition-colors"
                      >
                        {skill}
                      </span>
                    ))
                  ) : (
                    <p className="text-xs text-slate-500 italic">Nenhuma skill técnica listada explicitamente no documento.</p>
                  )}
                </div>
              </div>
                  ...
                  {candidate.experiences && candidate.experiences.length > 0 ? (
                    candidate.experiences.map((exp, i) => (
                      <div key={i} className="relative pl-9 group">
                        <span className="absolute left-2 top-2.5 w-3.5 h-3.5 rounded-full bg-slate-800 border-2 border-indigo-500 group-hover:scale-110 transition-transform" />
                        <div className="p-4 rounded-xl border border-slate-800 bg-slate-950/60 hover:bg-slate-950/80 transition-all shadow-sm">
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 mb-1">
                            <h4 className="font-bold text-sm text-slate-100 group-hover:text-indigo-300 transition-colors">
                              {exp.job_title}
                            </h4>
                            {exp.is_current && (
                              <span className="w-max text-[9px] font-extrabold bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 px-2 py-0.5 rounded-full uppercase tracking-tighter">
                                Atual
                              </span>
                            )}
                          </div>
                          <div className="text-xs text-indigo-400 font-semibold mb-2.5">{exp.company_name}</div>
                          {exp.description && (
                            <p className="text-xs text-slate-400 leading-relaxed font-normal bg-slate-900/40 p-2.5 rounded-lg border border-slate-800/40">
                              {exp.description}
                            </p>
                          )}
                        </div>
                    ...
                    <div className="pl-9 text-xs text-slate-500 italic py-2">
